...
// Updated type definitions based on your actual schema
type Project = {
  id: string
  slug: string
  client_name: string
...
function mapProjectToUI(project: Project): ProjectWithDetails {
  ...
  const metrics = project.metrics
    ...
    ?.flatMap(m => {
      try {
        const data = typeof m.metrics_data === 'string' 
...
export default async function ProjectPage({ params }: Props) {
  ...
  const nextProjectData = allProjects[(projectIndex + 1) % allProjects.length]
        ...
        <section className="px-4 pb-16 sm:px-6 sm:pb-24">
          <div className="mx-auto grid max-w-6xl gap-4 sm:grid-cols-3">
            {project.metrics.map((metric) => (
              <article key={metric.label} className="rounded-2xl border border-[#514d57] bg-[#111013] p-7">
            ...
            <div className="mt-8 grid gap-5 md:grid-cols-[1.5fr_1fr]">
              ...
              <div className="flex flex-col gap-4">
                {project.visualItems.map((item) => (
                  <div key={item.label} className="flex flex-1 items-center justify-between rounded-2xl border border-[#514d57] bg-[#0b0a0d] p-6">
      ...
      {project.services.length > 0 && (
        <section className="px-4 py-16 sm:px-6 sm:py-24">
          <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-2">
            <div>
              <p className="text-sm font-bold uppercase tracking-[0.2em] text-[#9a5cff]">Services delivered</p>
              <h2 className="mt-5 text-4xl font-black tracking-tight">From first workshop to successful launch.</h2>
            </div>
            <ul className="grid gap-3 sm:grid-cols-2">
              {project.services.map((service) => (
                <li key={service} className="flex items-center gap-3 rounded-xl border border-[#514d57] bg-[#111013] p-4 text-sm font-bold">
                  <span className="flex size-7 items-center justify-center rounded-full bg-[#6417ed]">
                    <Check size={15} />
                  </span>
                  {service}
                </li>
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              {project.techStack.map((tech) => (
                <span key={tech} className="rounded-full border border-[#514d57] bg-[#111013] px-4 py-2 text-sm font-bold">
                  {tech}
                </span>
              ))}
            </div>
